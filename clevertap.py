    ...
    def _fetch_records(self, type, query, batch_size=10):
        ...
        if self.cursor:
            ...
            def call_records():
                ...
                # write the records into a csv file here instead using
                # pandas

                df=pd.read_json(json.dumps(new_records), orient='records')

                filename = "./dumps/"+time.strftime("%Y%m%d-%H%M%S")+"___"+self.cursor+".csv";
                df.to_csv(filename)
